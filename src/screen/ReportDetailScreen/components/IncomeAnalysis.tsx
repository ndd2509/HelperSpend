...
const IncomeAnalysis = () => {
  ...
  useEffect(() => {
    const fetch = async () => {
      setLoading(true);
      try {
        const res = await getAIAnalysis({ type: 'income' });
  ...
  if (loading) {
    return (
      <View style={styles.loadingBox}>
        ...
        <Text style={styles.loadingText}>🤖 AI đang phân tích thu nhập...</Text>
      </View>
    );
  }

  const breakdown = chartData?.incomeBreakdown || [];
  ...
  return (
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.summaryRow}>
            ...
            <View style={[styles.summaryCard, { borderLeftColor: '#FF9500' }]}>
              ...
              <Text
                style={[
                  styles.summaryValue,
                  { color: summary.savingRate >= 0 ? '#34C759' : '#FF3B30' },
                ]}
              >
                {summary.savingRate}%
              </Text>
          ...
          <View style={styles.catList}>
            {breakdown.slice(0, 6).map((b: any, i: number) => (
              <View key={i} style={styles.catItem}>
                ...
                <Text style={[styles.catAmount, { color: '#34C759' }]}>
                  {fmt(b.amount)}
                </Text>
              </View>
            ))}
      ...
      {trend.length > 0 && (
        <View style={styles.chartCard}>
          <Text style={styles.chartTitle}>📈 Xu hướng thu nhập 3 tháng</Text>
          <View style={styles.trendRow}>
            {trend.map((m: any, i: number) => {
              const maxVal = Math.max(...trend.map((t: any) => t.income), 1);
